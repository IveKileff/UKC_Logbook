...
t_grade_dic = build_summary_dict(t_dates, trad_grade_order, r_trad)
t_grade_dict = rationalise_summary_dict(t_grade_dic)

# visualise sport climbs
counter = 30
x = s_dates
y_label = next(iter(s_grade_dict))
y = s_grade_dict[y_label]
fig = go.Figure(go.Bar(x =x, y=y, name=y_label, marker_color = 'rgb(190, 0, 30)'))
for key in s_grade_dict:
    if key == y_label:
        continue
    fig.add_trace(go.Bar(x=x, y=s_grade_dict[key], name=key, 
        marker_color = f'rgb(190, {counter}, 30)'))
    counter += 30
fig.update_layout(barmode='stack', title_text='Sport Routes', 
    xaxis_tickangle=-90)
fig.show()

# visualise trad climbs
x = t_dates
y_label = next(iter(t_grade_dict))
y = t_grade_dict[y_label]
fig = go.Figure(go.Bar(x =x, y=y, name=y_label))
for key in t_grade_dict:
    if key == y_label:
        continue
    fig.add_trace(go.Bar(x=x, y=t_grade_dict[key], name=key))
fig.update_layout(barmode='stack', title_text='Trad Routes', 
    xaxis_tickangle=-90)
fig.show()
...
fig = go.Figure(go.Bar(x =x, y=y, name=y_label, marker_color = 'rgb(190, 0, 30)'), row=1, col=1)
for key in s_grade_dict:
    if key == y_label:
        continue
    fig.add_trace(go.Bar(x=x, y=s_grade_dict[key], name=key, 
        marker_color = f'rgb(190, {counter}, 30)'), row=1, col=1)
    counter += 30
fig.update_layout(barmode='stack', title_text='Sport Routes', 
    xaxis_tickangle=-90)
...
y_label = next(iter(t_grade_dict))
y = t_grade_dict[y_label]
fig = go.Figure(go.Bar(x =x, y=y, name=y_label), row=1, col=2)
for key in t_grade_dict:
    if key == y_label:
        continue
    fig.add_trace(go.Bar(x=x, y=t_grade_dict[key], name=key), row=1, col=2)
fig.update_layout(barmode='stack', title_text='Trad Routes', 
    xaxis_tickangle=-90)
