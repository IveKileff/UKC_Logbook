...
def preps(df):
    ...
    # 2. I'd like to show which Lead styles have been kept and which dropped
    '''categorise climbs'''
    df1 = df1.assign(Climb_type = df1['Grade'])
    df1['Grade filter'] = df1['Grade'].str[0]
    df1.loc[df1['Grade filter'] == 'f', 'Climb_type'] = 'Boulder'
    df1.loc[df1['Grade filter'].str.contains('1|2|3|4|5|6|7|8|9'),
            'Climb_type'] = 'Sport'
    df1.loc[df1['Grade filter'].str.contains('M|D|S|H|V|E'), 'Climb_type'] = 'Trad'
    # 3. I'd like to print the set of 'Style' that I'm dropping as a check 
...
def plot_data(df, g_title, years):    
    ...
    mx = max(years) + 0.5
    mn = min(years) - 0.5
    fig = px.bar(df, x='Year', y='Count', color=df.index,
                 facet_col='Chart',
...
#4. sort out that the trad and sport charts appear in a different order
